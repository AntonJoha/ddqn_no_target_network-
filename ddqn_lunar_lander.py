...
class DDQNAgent:
    def __init__(self, state_dim: int, action_dim: int, config: DDQNConfig, device: torch.device):
        ...
        self.always_use_target_network = config.target_network_countdown < 0
    ...
    def train_step(self, replay_buffer: ReplayBuffer):
        ...
        with torch.no_grad():
            ...
            if self.always_use_target_network or self.use_target_network:
    ...
    def update_target_network_countdown(self):
        if self.always_use_target_network:
            return
